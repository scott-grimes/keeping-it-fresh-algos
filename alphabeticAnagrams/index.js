...
const anagramPosition = string => {
  ...
  let orig = string.split('');
  let uniques = Array.from(new Set(orig)).sort();
  let positionDict = {};
  ...
  const numerator = orig.map((c, index) => {
    ...
    stringAfterMe = orig.slice(index);
  ...
  const comboArr = numerator.map((x, i) => {
    return x * reverseFact[i] / denominator[i];
  })
